...
foreach ($donnees_s as $key => $value) {
    $bd = new PDO('mysql:host=localhost;dbname=Animath','root','');
    $req = $bd->prepare('INSERT INTO Stand(nom,description,capacite,journee,duree,inters,nbex_j,nbex_v,deb_dej,fin_dej,niv,jeudi,vendredi) VALUES (:nom,:description,:capacite,:journee,:duree,:inters,:nbex_j,:nbex_v,:deb_dej,:fin_dej,:niv,:jeudi,:vendredi)');
    foreach ($value as $k => $v) {
        if($k == 'Nb max de visiteurs'){
    ...
    elseif ($k == "pause_start") {
        $req->bindValue(':deb_dej', $v);
        echo 'ok';
    }
    elseif ($k == "pause_end") {
        $req->bindValue(':fin_dej', $v);
        echo 'ok';
    }
    elseif ($k == "nv_scol") {
        $req->bindValue(':niv', $v);
